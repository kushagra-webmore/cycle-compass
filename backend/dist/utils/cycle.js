...
export const derivePhase = (day, cycleLength, periodLength = DEFAULT_PERIOD_LENGTH) => {
...
export const getCycleContext = (startDate, cycleLength, userPeriodLength, referenceDate = new Date()) => {
    ...
    if (phase === 'MENSTRUAL') {
        ...
        if (nextPhaseStart >= fertileStart)
        ...
        if (currentDay < ovulationDay)
            nextPhaseStart = ovulationDay;
        else if (currentDay === ovulationDay)
            nextPhaseStart = ovulationDay + 1;
        else
            nextPhaseStart = fertileEnd + 1; // Start of Luteal
    }
