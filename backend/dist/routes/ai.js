...
import { aiExplainSchema, aiJournalSummarySchema, aiPartnerGuidanceSchema, } from '../validators/ai.js';
import { asyncHandler } from '../utils/async-handler.js';
import { generateCycleExplainer, generateJournalSummary, generatePartnerGuidance, generateDailyInsights, } from '../services/ai.service.js';
import { getCurrentCycle, getCycleById, getLatestSymptomEntry, } from '../services/cycle.service.js';
import { getPartnerSummary } from '../services/partner.service.js';
export const aiRouter = Router();
aiRouter.post('/daily-insights', authenticate, requireRoles('PRIMARY'), asyncHandler(async (req, res) => {
...
aiRouter.post('/explain', authenticate, requireRoles('PRIMARY'), validateBody(aiExplainSchema), asyncHandler(async (req, res) => {
    const userId = req.authUser.id;
...
aiRouter.post('/journal-summary', authenticate, requireRoles('PRIMARY'), validateBody(aiJournalSummarySchema), asyncHandler(async (req, res) => {
    const userId = req.authUser.id;
...
aiRouter.post('/partner-guidance', authenticate, requireRoles('PARTNER'), validateBody(aiPartnerGuidanceSchema), asyncHandler(async (req, res) => {
