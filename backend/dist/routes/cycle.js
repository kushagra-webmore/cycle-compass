...
import { createCycleSchema, bulkCycleSchema, logSymptomSchema, updateCycleSchema } from '../validators/cycle.js';
...
cycleRouter.post('/', validateBody(createCycleSchema), asyncHandler(async (req, res) => {
    const cycle = await createCycle(req.authUser.id, req.body);
    res.status(201).json(cycle);
}));
cycleRouter.post('/bulk', validateBody(bulkCycleSchema), asyncHandler(async (req, res) => {
    const result = await createCyclesBulk(req.authUser.id, req.body.cycles);
    res.status(201).json({ cycles: result });
}));
cycleRouter.get('/history', asyncHandler(async (req, res) => {
    const cycles = await getCyclesHistory(req.authUser.id);
    res.json(cycles);
}));
cycleRouter.patch('/:id', validateBody(updateCycleSchema), asyncHandler(async (req, res) => {
    const cycle = await updateCycle(req.authUser.id, req.params.id, req.body);
    res.json(cycle);
}));
cycleRouter.delete('/:id', asyncHandler(async (req, res) => {
    await deleteCycle(req.authUser.id, req.params.id);
    res.json({ success: true });
}));
cycleRouter.get('/current', asyncHandler(async (req, res) => {
    const cycle = await getCurrentCycle(req.authUser.id);
    res.json(cycle);
}));
symptomRouter.post('/log', validateBody(logSymptomSchema), asyncHandler(async (req, res) => {
    const { cycleId, ...symptom } = req.body;
...
symptomRouter.get('/history', asyncHandler(async (req, res) => {
