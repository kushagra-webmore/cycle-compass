...
if (process.env.VAPID_PUBLIC_KEY && process.env.VAPID_PRIVATE_KEY) {
    webpush.setVapidDetails('mailto:noreply@cyclecompass.com', process.env.VAPID_PUBLIC_KEY, process.env.VAPID_PRIVATE_KEY);
}
else {
...
export class NotificationService {
    ...
    static async sendNotification(userId, payload) {
        ...
        const notifications = subs.map(sub => {
            return webpush.sendNotification({ endpoint: sub.endpoint, keys: sub.keys }, JSON.stringify(payload)).catch(err => {
