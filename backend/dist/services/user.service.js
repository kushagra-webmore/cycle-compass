...
const mapUserRowToAuthUser = (row, email) => {
...
export const getUserWithProfile = async (userId, email) => {
    const supabase = getSupabaseClient();
    const { data, error } = await supabase
        .from('users')
        .select('id, role, status, last_login, last_activity, profiles(name, age, date_of_birth, phone, city, timezone, onboarding_completed, last_period_date, cycle_length, period_length, goal)')
        .eq('id', userId)
        .single();
    if (error || !data) {
